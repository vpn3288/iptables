...
# [A12] save_rules: systemd 服务创建后只 enable 未 try-restart，
#       服务处于 failed 状态时不会被拉起
...
SSH_PORT="" OPEN_PORTS=() HOP_RULES=() VERSION="4.3" DRY_RUN=false
...
# port.sh v4.3 — 与 youhua.sh v2.4 / BBRplus 完全兼容
...
detect_existing_hop_rules() {
    while IFS= read -r line; do
        ...
        range=$(echo "$line" | grep -oE 'dpts:[0-9]+:[0-9]+' \
            | grep -oE '[0-9]+:[0-9]+' | tr ':' '-')
        # 兼容三种 iptables 显示格式:
        #   to::443              （iptables DNAT --to-destination :443）
        #   to:0.0.0.0:443       （--to-destination 0.0.0.0:443）
        #   to:[::]:443          （ip6tables DNAT --to-destination [::]:443）
        target=$(echo "$line" \
...
apply_hop() {
    ...
    # ── INPUT 链放行跳跃端口范围 ───────────────────────────────
    # DNAT 目标是本机（to-destination 不改变 IP，只改端口），
    # 经 PREROUTING 后目标地址仍是本机，走 INPUT 而非 FORWARD
    iptables -C INPUT -p udp --dport "${s}:${e}" -j ACCEPT 2>/dev/null \
        || iptables -A INPUT -p udp --dport "${s}:${e}" -j ACCEPT 2>/dev/null || true
    iptables -C INPUT -p tcp --dport "${s}:${e}" -j ACCEPT 2>/dev/null \
        || iptables -A INPUT -p tcp --dport "${s}:${e}" -j ACCEPT 2>/dev/null || true
    ...
    if command -v ip6tables &>/dev/null; then
        ...
        # [A1 FIX] ip6tables DNAT 必须用 "[::]:PORT" 格式
        #          ":PORT" 在 ip6tables 中是非法 IPv6 地址，会静默失败
...
# [A12 FIX] systemd 服务创建后 try-restart，处理 failed 状态
# ============================================================
save_rules() {
...
SVC
        ...
        # [A12 FIX] try-restart 处理 failed 状态；首次运行时 start 也包含在内
        systemctl try-restart iptables-restore.service &>/dev/null || true
    fi
    ok "规则已保存，重启后自动生效"
}
