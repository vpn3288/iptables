...
# 角色变量（由参数或交互设置）
# ROLE: auto | relay | landing
ROLE="auto"

# 中转机专用变量（--relay 模式或 zhongzhuan.sh 调用时传入）
RELAY_WG_PORT=""    # WireGuard UDP 端口
RELAY_MACKA_PORT="" # Mack-a TCP 端口
RELAY_WG_SUBNET=""  # WireGuard 内网段，默认 10.0.0.0/24
WG_IFACE="wg0"      # WireGuard 网卡名
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        # 中转机参数（供 zhongzhuan.sh 直接调用时传入）
        --wg-port)    shift; RELAY_WG_PORT="$1" ;;
        --macka-port) shift; RELAY_MACKA_PORT="$1" ;;
        ...
        --help|-h)
            ...
            echo "  --relay            中转机模式"
            echo "  --landing          落地机模式"
            echo "  --wg-port PORT     指定 WireGuard 端口（中转机模式）"
            echo "  --macka-port PORT  指定 Mack-a 端口（中转机模式）"
...
# rp_filter=2：宽松模式，兼容端口跳跃 UDP 转发
...
detect_role() {
    ...
    # 判断逻辑（优先级从高到低）：
    # 1. 有 v2ray-agent 代理软件 => 落地机
    # 2. wg0.conf 有 ListenPort 且有多个 Peer => 中转机
    # 3. wg0.conf 只有一个 Peer => 落地机（连接到中转机）
    # 4. 默认 => 落地机
    ...
    if [[ -f /etc/wireguard/wg0.conf ]]; then
        ...
        if [[ "$has_listen" -gt 0 && "$peer_count" -gt 1 ]]; then
            ROLE="relay"
            ok "自动检测角色: 中转机 (relay) — wg0.conf 含 $peer_count 个 Peer"
        else
            ROLE="landing"
            ok "自动检测角色: 落地机 (landing) — wg0.conf 含 $peer_count 个 Peer"
        fi
        return
    fi
...
detect_relay_params() {
    # 如果已由参数传入则跳过
    if [[ -n "$RELAY_WG_PORT" && -n "$RELAY_MACKA_PORT" ]]; then
        ok "使用传入参数 — WG端口: $RELAY_WG_PORT  Mack-a端口: $RELAY_MACKA_PORT"
    ...
    if [[ -f "$wg_conf" ]]; then
        ...
        [[ -z "$RELAY_WG_SUBNET" ]] && \
            RELAY_WG_SUBNET=$(grep -E '^\s*Address' "$wg_conf" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/[0-9]+' | head -1 || true)
        # 把落地机段提取出来（把末位改成0）
        if [[ -n "$RELAY_WG_SUBNET" ]]; then
            local base; base=$(echo "$RELAY_WG_SUBNET" | cut -d'/' -f1 | sed 's/\.[0-9]*$/\.0/')
            local mask; mask=$(echo "$RELAY_WG_SUBNET" | cut -d'/' -f2)
            RELAY_WG_SUBNET="${base}/${mask}"
        fi
    fi

    local macka_conf=/etc/mack-a/server.conf
    if [[ -z "$RELAY_MACKA_PORT" && -f "$macka_conf" ]]; then
        RELAY_MACKA_PORT=$(grep -E '^\s*listen' "$macka_conf" \
            | grep -oE ':[0-9]+' | grep -oE '[0-9]+' | head -1 || true)
    fi

    RELAY_WG_PORT="${RELAY_WG_PORT:-51820}"
    RELAY_MACKA_PORT="${RELAY_MACKA_PORT:-8080}"
    RELAY_WG_SUBNET="${RELAY_WG_SUBNET:-10.0.0.0/24}"

    ok "中转机参数 — WG端口: $RELAY_WG_PORT  Mack-a端口: $RELAY_MACKA_PORT  WG内网: $RELAY_WG_SUBNET"
}
...
apply_rules() {
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        if [[ "$ROLE" == "relay" ]]; then
            info "WG 端口    : $RELAY_WG_PORT/UDP"
            info "Mack-a 端口: $RELAY_MACKA_PORT/TCP"
            info "WG 内网    : $RELAY_WG_SUBNET"
        else
    ...
    # SSH 防暴力（recent 模块：60秒内超6次新连接则丢弃）
    ...
    if [[ "$ROLE" == "relay" ]]; then
        ...
        # Mack-a TCP 入站
        iptables -A INPUT -p tcp --dport "$RELAY_MACKA_PORT" -j ACCEPT
        ok "放行 Mack-a: TCP $RELAY_MACKA_PORT"

        # WireGuard 内网 FORWARD（落地机之间互通 + 回包）
        iptables -A FORWARD -i "$WG_IFACE" -j ACCEPT
        iptables -A FORWARD -o "$WG_IFACE" -j ACCEPT
        iptables -A FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
        ok "放行 FORWARD: $WG_IFACE 双向"

        # NAT MASQUERADE（WireGuard 内网流量出口伪装）
        iptables -t nat -A POSTROUTING -s "$RELAY_WG_SUBNET" -o "$wan_iface" -j MASQUERADE
        # 同时为 wg 出口做 MASQUERADE（peer 互访）
        iptables -t nat -A POSTROUTING -s "$RELAY_WG_SUBNET" -o "$WG_IFACE" -j MASQUERADE
        ok "启用 NAT MASQUERADE: $RELAY_WG_SUBNET → $wan_iface / $WG_IFACE"

        # 可选：如果本机同时跑了代理服务，也放行检测到的端口
        ...
        ok "开放端口: ${OPEN_PORTS[*]:-无}"

        # 如果落地机通过 WireGuard 连接中转机，放行 wg 接口双向 FORWARD
        if ip link show "$WG_IFACE" &>/dev/null 2>&1; then
            iptables -A FORWARD -i "$WG_IFACE" -j ACCEPT
            iptables -A FORWARD -o "$WG_IFACE" -j ACCEPT
            ok "落地机 WireGuard FORWARD: $WG_IFACE 双向"
        fi

        # FORWARD：ESTABLISHED 和 DNAT 回包
        iptables -A FORWARD -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
        iptables -A FORWARD -m conntrack --ctstate DNAT -j ACCEPT

        # NAT MASQUERADE（落地机出口）
        iptables -t nat -A POSTROUTING -o "$wan_iface" -j MASQUERADE
        ok "启用 NAT MASQUERADE: → $wan_iface"
...
show_status() {
    ...
    echo -e "\n${G}▸ FORWARD 规则:${W}"
    iptables -L FORWARD -n 2>/dev/null | grep ACCEPT \
        | sed 's/^/  • /' || echo "  无"
...
show_summary() {
    ...
    if [[ "$ROLE" == "relay" ]]; then
        ...
        echo -e "${C}WG 端口    :${W} $RELAY_WG_PORT/UDP"
        echo -e "${C}Mack-a 端口:${W} $RELAY_MACKA_PORT/TCP"
        echo -e "${C}WG 内网    :${W} $RELAY_WG_SUBNET (FORWARD + MASQUERADE 已启用)"
        if [[ ${#OPEN_PORTS[@]} -gt 0 ]]; then
            echo -e "${C}额外端口   :${W} ${OPEN_PORTS[*]}"
        fi
    else
        ...
        if [[ ${#HOP_RULES[@]} -gt 0 ]]; then
            ...
            warn "未配置端口跳跃  →  如需添加: bash port.sh --add-hop"
        fi
    fi
    hr
    echo -e "${Y}常用命令:${W}"
...
main() {
    ...
    if [[ "$ROLE" == "relay" ]]; then
        ...
        info "Mack-a 端口: $RELAY_MACKA_PORT/TCP"
        info "WG 内网    : $RELAY_WG_SUBNET"
        [[ ${#OPEN_PORTS[@]} -gt 0 ]] && info "额外端口   : ${OPEN_PORTS[*]}"
        echo
        ...
        [[ ${#HOP_RULES[@]} -eq 0 ]] && warn "未检测到端口跳跃  →  如需添加: bash port.sh --add-hop"
        echo
    fi
