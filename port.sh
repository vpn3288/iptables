...
show_help() {
    cat << 'EOF'
精准代理端口防火墙管理脚本 v2.0.1 (iptables版)
...
detect_existing_nat_rules() {
    ...
    # 检测iptables NAT规则（增强版）
    if command -v iptables >/dev/null 2>&1; then
        debug_log "扫描iptables PREROUTING NAT规则..."
    ...
    if [ ${#NAT_RULES[@]} -gt 0 ]; then
        ...
        for rule in "${NAT_RULES[@]}"; do
            ...
            echo -e "  ${CYAN}• 端口范围: $port_range (${range_size}个端口) → 目标: $target_port${RESET}"
...
detect_listening_ports() {
    ...
    while IFS= read -r line; do
        if [[ "$line" =~ LISTEN ]] || [[ "$line" =~ UNCONN ]]; then
            ...
            # 提取端口号
            local port=$(echo "$address_port" | grep -oE '[0-9]+ then
                        while read -r port; do
                            config_ports+=("$port")
...
is_standard_proxy_port() {
    ...
    # 检查高端口范围（30000-39999, 40000-65000）- 但排除已知内部端口
