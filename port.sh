...
install_deps() {
    ...
    # 生成 JSON 端口解析辅助脚本（过滤 listen=127.x 的内部 inbound）
    cat > /tmp/_parse_ports.py << 'PYEOF2'
import json, sys
try:
    ...
    for inb in inbounds:
        ...
        port   = inb.get("port")
        listen = str(inb.get("listen", ""))
        if not port: continue
        if listen.startswith("127.") or listen == "::1": continue
        print(port)
except Exception:
    pass
PYEOF2
...
detect_config_ports() {
    ...
    # 从配置文件提取端口，过滤 listen=127.x 的内部端口
    for f in "${configs[@]}"; do
        [[ -f "$f" ]] || continue
        local port_list=""

        if command -v python3 &>/dev/null; then
            # python3 精确解析 JSON，跳过 listen=127.x 的 inbound
            port_list=$(python3 /tmp/_parse_ports.py "$f" 2>/dev/null || true)
        fi

        # 回退：grep 提取所有端口（不含 127.x 同行的端口）
        if [[ -z "$port_list" ]]; then
            port_list=$(grep -oE '"port"[[:space:]]*:[[:space:]]*[0-9]+' "$f" 2>/dev/null \
                | grep -oE '[0-9]+$' || true)
        fi
